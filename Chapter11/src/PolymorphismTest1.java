 ...
 * 설명 :  예제 11.4
 * 			다음 프로그램은 세 개의 클래스가 상속 관계에 있으면서 하나의 속성과 하나의 메소드를 가지고있습니다.
...
public class PolymorphismTest1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("r.count 값 : " + r.count); // Am의 카운트값 1출력.
			
		r = new Bm(); //형변환. 상위클래스 형 객체 변수에 하위 클래스 객체
		r.callme(); //Bm에 있는 callme()호출.
		System.out.println("r.count 값 : " + r.count); // Am에 있는 count 출력
		
		/*
		 * 형변환의 경우 메소드는 오버라이딩된 결과를 적용할 수 있다.
		 * 즉, 하위 클래스의 메소드에 접근이 가능하다.
		 * 
		 * 현변환의 경우 속성(변수)는 오버라이딩 되지 않는다.
		 * 따라서 상위 클래스의 속성에 접근한다.
		 */
		
		r = new Cm();
		r.callme();	// Cm에 있는 callme호출
		System.out.println("r.count 값 : " + r.count); // 결과 : Am의 count출력
	}
